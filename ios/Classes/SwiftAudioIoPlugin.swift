...
class _Constants {
    ...
    static let workspaceSamples = 100000
...
public class SwiftAudioIoPlugin: NSObject, FlutterPlugin {
    ...
    private lazy var sinkNode = AVAudioSinkNode() { (timeStamp, frames, audioBufferList) ->
        OSStatus in
        ...
        let data = Data.init(fromArray:doubleSamples)
    ...
    @objc func handleConfigChange(notification: NSNotification) {
...
public struct RingBuffer<T> {
    ...
    public mutating func writeBlock(_ block: [T?]) -> Bool {
        ...
        if(availableSpaceForWriting >= count){
            for i in 0..<count {
                array[(writeIndex+i) % array.count] =  block[i];
            }
            writeIndex += count
            return true
        }
